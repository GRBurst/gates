...
{	
    ...
    if (uRadius > 82 )
    	color = vec4(cos(fUV.x * 6.282) * 0.05 + 0.2314, cos(fUV.x * 6.282) * 0.12 + 0.4369, cos(fUV.x * 6.282) * 0.2666 + 0.7333, 1.0);
    //	color = mix(vec4(cos(fUV.x * 6.282) * 0.05 + 0.2314, cos(fUV.x * 6.282) * 0.12 + 0.4369, cos(fUV.x * 6.282) * 0.2666 + 0.7333, 1.0), vec4(texture(textureSky, vec3(fUV, uTime / 64)).rrr, 1), smoothstep(0.6, 0.9, texture(textureSky, vec3(fUV, uTime / 64)).r) );
    	...
    	{ 
    		if (testVal > 0.4)
    			color = vec4(texture(textureSky, vec3(fUV, uTime / 96)).rrr, 0.5);
    		else
    			color = vec4(0, 0, 0, 0);
    	}
    	else
    	{ 
    		if (testVal > 0.5)
    			color = vec4(texture(textureSky, vec3(fUV, uTime / 64)).rrr, 0.5);
    		else
    			color = vec4(0, 0, 0, 0);
    	}
